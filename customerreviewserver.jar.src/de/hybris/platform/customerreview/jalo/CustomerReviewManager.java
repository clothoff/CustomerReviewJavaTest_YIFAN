...
/*     */   }
			...
			public Integer getNumberOfReviewsWithinRange(SessionContext ctx, Product item, Double low, Double high)
			{
				String query = "SELECT count(*) FROM {" + GeneratedCustomerReviewConstants.TC.CUSTOMERREVIEW + "} WHERE {" + 
					"product" + "} = ?product AND {" + "rating" + "} BETWEEN" + low.toString() + "AND" + high.toString();
